...
window.PADDING_TOP_DIFF = 8;
const PADDING_VERTICAL = 90;
...
class S {
  constructor(opts) {
    ...
    this.current = -1;
    this.images = [];
    this.imagesToLoad = new Array(this.config.list.length).fill(false);
    this.directionSign = /-/.test(this.config.direction) ? -1 : 1;
    this.lastLoaded = -1;
    this.lastRendered = -1;
    this.lastRequstedRendered = -1;
    this.pongSign = 1;
  ...
  load() {
    ...
    new Preloader(
      ...
      this.config.sequenceName
    );

    function renderLoadingFrame() {
      if (self.hasBeenInteracted) return;

      const lastUnloaded =
        self.imagesToLoad.findIndex((e) => !e) === -1
          ? self.imagesToLoad.filter((e) => !!e).length
          : self.imagesToLoad.findIndex((e) => !e);
      const loadedAmount = self.imagesToLoad.filter((e) => !!e).length;
      const currentTime = new Date().getTime();

      // startRenderTime

      if (loadedAmount && lastUnloaded > self.lastRendered) {
        const averageFrameLoadSpeed = (currentTime - self.startRenderTime) / loadedAmount;

        self.renderSpeed = Math.max(Math.round((self.renderSpeed + averageFrameLoadSpeed) / 2), 16) + 4;

        self.lastRendered = Math.max(self.lastRendered + 1, 0);
        self.drawImage(self.lastRendered - 1);
      }

      if (self.lastRendered < self.config.list.length) {
        window.setTimeout(renderLoadingFrame, self.renderSpeed);
      }
    }
    window.requestAnimationFrame(renderLoadingFrame);
  }
  ...
  size(w, h) {
    ...
    this.config.canvas.parentElement.parentElement.style.marginTop = `${
      Math.round((window.innerHeight - height) / 2) - window.PADDING_TOP_DIFF
    }px`;
    this.config.canvas.parentElement.parentElement.style.marginLeft = `${Math.round(
...
function renderMove(self, id) {
  self.lastRequstedRendered = id;
  // console.log(id)
  self.drawImage(id);
  self.current = id;
...
function absoluteMove(self, e) {
  ...
  const movePadding = 0.08;
  const expectedId = Math.round((m / w) * t);
  const id = constrain(Math.round(expectedId * (1 + movePadding) - t * (movePadding / 2)), 0, t - 1);
...
function Preloader(arrayToPopulate, fileList, imageLoadCallback, queueCompleteCallbak, sequenceName) {
    ...
    (function (id) {
      ...
      img.onload = (e) => {
        ...
        if (count < fileList.length) {
          if (sequenceName && sequenceName !== window.CURRENT_INSTANCE) {
            // console.log('!', sequenceName, window.CURRENT_INSTANCE);
